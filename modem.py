    ...
    def set_cards(self, in_card, out_card):
        if in_card or out_card:
            ...
            try: # We try 44100 and 48000 and give up after that
                valid = self.p.is_format_supported(48000,
            ...
            self.stream = self.p.open(format=self.p.get_format_from_width(self.WIDTH),
                    channels=1, rate=self.RATE, output=(out_card != None), input=(in_card != None),
                stream_callback=self.pacallback, frames_per_buffer=self.FRAMESPERBUFFER, input_device_index=in_card, output_device_index=out_card )
        else:
            if self.stream:
                self.stream.stop_stream()
            self.stream = None
